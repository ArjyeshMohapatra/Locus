...
## System Status
- `GET /health`
  - Returns component status (DB, Watcher, Snapshot Engine)
...
- `POST /snapshots/execute-action`
  - Execute the inferred action for a snapshot result (open URL/file/app).
  - Body: `{ "action_type": "open_url|open_file|open_app", "value": "..." }`
- `DELETE /snapshots/{id}`
