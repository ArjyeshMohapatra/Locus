...
def setup():
    ...
    # ensure it's watched
    requests.post(f"{API_URL}/files/watched", json={"path": TEST_FOLDER}, timeout=10)
...
def main():
    ...
    resp = requests.get(
        f"{API_URL}/files/versions", params={"path": TEST_FILE}, timeout=10
    )
    versions = resp.json()
    ...
    restore_resp = requests.post(
        f"{API_URL}/files/restore", json={"version_id": v1["id"]}, timeout=10
    )
