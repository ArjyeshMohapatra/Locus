...
def test_backup_task_failed_records_error(tmp_path: Path):
    ...
    try:
        ...
        if task.status != "failed":
            fail(f"Expected status to be 'failed', got {task.status}")
        if task.last_error != "boom":
            fail(f"Expected error to be 'boom', got {task.last_error}")
        if task.attempts != 1:
            fail(f"Expected attempts to be 1, got {task.attempts}")
    finally:
        db.close()
