...
<!-- Modal Dialog -->
<div class="modal show" tabindex="-1" role="dialog" aria-modal="true" on:click|self={onClose} on:keydown={(e) => e.key === 'Escape' && onClose()} transition:fade={{ duration: 200 }}>
    <div class="modal-dialog modal-xl" role="document" transition:scale={{ duration: 200, start: 0.95 }}>
    <div class="modal-content shadow-lg glass-modal">
            <div class="modal-header modal-header-shell {selectedVersion ? 'preview-header' : 'history-header'}">
                ...
                <div class="header-right">
                    <div class="preview-controls-shell {selectedVersion ? 'is-visible' : 'is-hidden'}" aria-hidden={!selectedVersion}>
                        {#if selectedVersion}
                    ...
                    <button type="button" class="btn-close modal-close-btn" aria-label="Close modal" on:click={onClose}></button>
                </div>
      </div>
      ...
      <div class="modal-body">
        ...
        <div class="mode-stage">
          {#key selectedVersion ? `preview-${selectedVersion.id}` : 'history'}
            <div class="history-content-region mode-pane" transition:fade|local={{ duration: 200 }}>
              {#if selectedVersion}
                  <!-- Preview View -->
...
<style>
    ...
    .preview-controls-shell.is-visible {
        opacity: 1;
        transform: translateY(0);
    }
    .preview-controls-shell.is-hidden {
        opacity: 0;
    ...
    :global(body.locus-modal-open .titlebar) {
        display: none !important;
    }

    :global(body.has-custom-titlebar.locus-modal-open .app-shell) {
        margin-top: 0 !important;
        height: 100vh !important;
    }

    :global(body.has-custom-titlebar.locus-modal-open .app-container) {
    ...
    @media (max-width: 992px) {
        ...
        .modal-header-shell.history-header .header-right {
            width: auto;
            justify-content: flex-end;
            flex: 0 0 auto;
        }
